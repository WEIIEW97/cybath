...
namespace LOESS {
  struct Point {
    ...
    Point() : dcomp(), dval(0), drw(1){};
    Point(const Point& p)
        : dcomp(p.begin(), p.end()), dval(p.val()), drw(p.rw()){};
    template <class inputIt>
    Point(inputIt beg, inputIt past) : dcomp(beg, past), dval(0), drw(1) {}
    template <class inputIt>
    Point(inputIt beg, inputIt past, double& val)
        : dcomp(beg, past), dval(val), drw(1) {}
    Point(std::size_t const& n, Cor c) : dcomp(n, c), dval(0), drw(1){};
    ...
    bool operator==(Point const& p) const {
      return (std::equal(begin(), end(), p.begin()) && val() == p.val() &&
              rw() == p.rw());
    }
...
namespace CGAL {
  template <>
  struct Kernel_traits<LOESS::Point> {
    struct Kernel {
      typedef double FT;
      typedef double RT;
    };
