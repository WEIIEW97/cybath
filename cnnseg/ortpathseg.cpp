...
void padding_resize(cv::InputArray& src, cv::OutputArray& dst, float& fratio, int& ipadd_w, int& ipadd_h, cv::Size size, double fx = 0, double fy = 0, int interpolation = cv::INTER_LINEAR)
{
    float padd_w = 0;
    float padd_h = 0;
    ...
    cv::copyMakeBorder(dst, dst, top, bottom, left, right, cv::BORDER_CONSTANT, cv::Scalar(0, 0, 0));//top, bottom, left, right分别表示在原图四周扩充边缘的大小
...
void unpadding_resize(Mat& src, Mat& dst, int ipadd_w, int ipadd_h, int iraw_w, int iraw_h, double fx = 0, double fy = 0, int interpolation = cv::INTER_LINEAR)
{
    Mat cropimg;
...
void run_ort_trt(Mat& img, Mat& mask) {
...
#else
    const char* model_path = "/algdata01/huan.wang/samlabel/playground/mmsegmentation/work_dirs/ocrnet_hr18_4xb4-80k_ade20k-512x512/end2end.onnx";
#endif
    ...
    Mat blob = dnn::blobFromImage(img, 1., Size(img.rows, img.cols), Scalar(0, 0, 0), false, true);

    std::vector<int64_t> input_node_dims_real = { 1,3,img.rows, img.cols };
    
    input_tensors.emplace_back(Ort::Value::CreateTensor<float>(memory_info, blob.ptr<float>(), blob.total(), input_node_dims_real.data(), input_node_dims.size()));
...
int main(int argc, char* args) {
    cv::Mat src = cv::imread("C:/Users/Administrator/Desktop/road/000014.jpg");//720*405
    cv::Mat mask, dst;
    int ipadw, ipadh;
    float fratio;
    ...
    // padding and resize
    padding_resize(preprocimg, dst, fratio, ipadw, ipadh, Size(512, 512), 0, 0, cv::INTER_LINEAR);
    ...
    // 
    Mat unpadmask;
    unpadding_resize(mask, unpadmask, ipadw, ipadh, src.cols, src.rows, 0, 0, cv::INTER_LINEAR);
