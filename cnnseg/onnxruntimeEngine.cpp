...
int OnnxRuntimeEngine::paddingResize(cv::InputArray& src, cv::OutputArray& dst, float& fratio, int& pad_w, int& pad_h, cv::Size size, double fx, double fy, int interpolation, cv::Scalar padval)
{
	float fpadd_w = 0;
	float fpadd_h = 0;
	//Here, we first find the one with the longest edge in the original image. 
	//Then we resize it according to the longest edge, and fill in the smaller ones.
	...
	//top, bottom, left, right indicate the size of the expanded edges around the original image
	cv::copyMakeBorder(dst, dst, top, bottom, left, right, cv::BORDER_CONSTANT, padval);
...
int OnnxRuntimeEngine::unpaddingResize(Mat& src, Mat& dst, int pad_w, int pad_h, int raw_w, int raw_h, double fx, double fy, int interpolation)
{
	Mat cropimg;
...
int OnnxRuntimeEngine::processSeg(Mat& img, Size targetSize, int &padw,int &padh, void* output_buffer)
{
	int res;
	...
	res = paddingResize(dst, preprocimg, fratio, padw, padh, targetSize, 0, 0, cv::INTER_LINEAR);
	if (res != ONNXRUNTIMEENGINE_SUCCESS)
		return res;

	//Load image
	Mat blob = dnn::blobFromImage(preprocimg, 1., Size(preprocimg.rows, preprocimg.cols), Scalar(0, 0, 0), false, true);
	...
	CheckStatus(g_ort, g_ort->GetTensorMutableData(output_tensors[0], &local_output_buffer));

	memcpy(output_buffer, local_output_buffer, preprocimg.rows * preprocimg.cols * sizeof(int64_t));

	//Mat finalmask;
	//
	//res = segmentationPost(local_output_buffer, finalmask, preprocimg.rows, preprocimg.cols, padw, padh, img.rows, img.cols, 1);
	//if (res != ONNXRUNTIMEENGINE_SUCCESS)
	//	return res;
...
int OnnxRuntimeEngine::processDet(Mat& img, Size targetSize, long long maxclassid, float threshold, std::vector<BoundingBox>& outboxes)
{
	int res;
	const OrtApi* g_ort = (const OrtApi*)g_ort_rt;
	...
	res = paddingResize(img, preprocimg, fratio, padw, padh, targetSize, 0, 0, cv::INTER_LINEAR, cv::Scalar(114.0, 114.0, 114.0));
	if (res != ONNXRUNTIMEENGINE_SUCCESS)
		return res;
	...
	dets_buffer = NULL;
	CheckStatus(g_ort, g_ort->GetTensorMutableData(output_tensors[0], &dets_buffer));

	labels_buffer = NULL;
	CheckStatus(g_ort, g_ort->GetTensorMutableData(output_tensors[1], &labels_buffer));

	res = detectionPost(dets_buffer, labels_buffer, outboxes, fratio, 100, maxclassid, img.cols, img.rows, threshold);
	if (res != ONNXRUNTIMEENGINE_SUCCESS)
		return res;
...
int OnnxRuntimeEngine::classificationPost(void* output_buffer, size_t output_data_size)
{
...
int OnnxRuntimeEngine::segmentationPost(void* output_buffer, Mat& mask, int exrows, int excols, int expadw, int expadh, int rawrows, int rawcols, int64_t objlabel)
{
	...
	// 
	Mat unpadmask;
	unpaddingResize(exmask, unpadmask, expadw, expadh, rawcols, rawrows, 0, 0, cv::INTER_LINEAR);
...
int OnnxRuntimeEngine::detectionPost(void* dets_buffer, void* labels_buffer, std::vector<BoundingBox>& outboxes, float fratio, int outboxnum, long long maxclassid, int rawcols, int rawrows, float threshold)
{
	// Get pointer to output tensor float values
	...
	for (int i = 0; i < outboxnum; i++)
	{
		...
		box.x1 = valclamp((float)((floatarr[idx2++]) / fratio), (float)0.0, (float)(rawcols-1));
		box.y1 = valclamp((float)((floatarr[idx2++]) / fratio), (float)0.0, (float)(rawrows - 1));
		box.x2 = valclamp((float)((floatarr[idx2++]) / fratio), (float)0.0, (float)(rawcols - 1));
		box.y2 = valclamp((float)((floatarr[idx2++]) / fratio), (float)0.0, (float)(rawrows - 1));
		...
		//if (firstdraw)
		//{
		//	drawboxsave("./000000.jpg", "./image_with_rectangle.jpg", (int)box.x1, (int)box.y1, (int)box.x2, (int)box.y2);
		//	firstdraw = false;
		//}
		//else
		//{
		//	drawboxsave("./image_with_rectangle.jpg", "./image_with_rectangle.jpg", (int)box.x1, (int)box.y1, (int)box.x2, (int)box.y2);
		//}
...
int OnnxRuntimeEngine::drawboxsave(std::string imagename, std::string savepath, int x1, int y1, int x2, int y2)
{
	// 读取图像
	...
	// 定义框的颜色
	cv::Scalar color(0, 0, 255); // BGR格式，这里是红色
